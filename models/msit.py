...
from test_mod import amsno, MsC, RiM
import models
from models import register
from utils import make_coord

@register('amsit')
class amsit(nn.Module):
    def __init__(
            self,
            encoder_spec,
            imnet_spec,
        ...
        self.MsC_heads = MsC_heads
        self.MsSA_q = MsSA_q

        self.encoder = models.make(encoder_spec)

        self.conv_ch = nn.Conv2d(self.encoder.out_dim, self.dim, kernel_size=1)
        self.amsno = amsno(dim = self.dim, ca_num_heads=self.MsC_heads, use_lem=self.use_lem)

        self.conv_vs = nn.ModuleList([
            RiM(self.dim, self.dim, kernel_size=3, stride=1, padding=1, groups=1, map_k=3)
        ]) #nn.Conv2d(self.dim, self.dim, kernel_size=3, padding=1) for _ in range(self.conv_num)

        if self.local_attn:
            self.multi_conv_qs = nn.ModuleList([
                MsC(midc=self.dim,num=self.MsSA_q)
    ...
    def gen_feat(self):
        if self.prev_feat is None:
            ...
            self.feat = self.amsno(self.feat)
            self.init_feat = self.feat.clone()
        else:
            self.feat = self.prev_feat.clone()

        if self.local_attn:
            self.feat_q = self.multi_conv_qs[self.conv_idx](self.feat)
    ...
    def query_rgb(self, sample_coord, cell=None):
        ...
        if self.local_attn:
            # b, q, h, 1, r_area -> b, q, r_area, h k=[16, 2304, 8, 32, 49]

            attn = torch.matmul(sample_feat_q, sample_feat_qt).reshape(
    ...
    def chop_forward(self, inp, coords, cell, eval_bsize=100000, visual_indices=None):  # demo map
        self.inp = inp
